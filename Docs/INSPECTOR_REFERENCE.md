...
- `HostMenu` is now only the active Host ritual lobby. Successful creation silently changes the existing Book content to `Enter the Circle`, `Invite a Priest`, and `Quit Ritual`; it does not replay the Book transition or start ritual gameplay.
- Recreating the Book controller while an authoritative Host is already running restores that same active Host presentation through the silent content path. It must not call `ChangePage(HostMenu)` or retain cached transition targets. After `Quit Ritual`, the prepared state is `PlayMenu`, pending Host creation is cleared, and every later `Create Ritual` cycle follows the same silent success path as the first.
